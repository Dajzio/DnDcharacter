...
func main() {
	...
	switch cmd {
	case "create":
		handleCreate(ctx, charRepo)
	case "list":
		handleList(ctx, charRepo)
	case "view":
		handleView(ctx, charRepo)
	case "delete":
		handleDelete(ctx, charRepo)
	case "equip":
		handleEquip(ctx, charRepo)
	case "learn-spell":
		handleLearnSpell(ctx, charRepo, spellRepo)
	case "prepare-spell":
